Shader "Unlit/Water_Lab" //Shader路径名
{
    Properties //材质面板参数
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(VertexOutput i) : COLOR //像素shader
            {
                ...
                float4 waterColor2 = lerp(waterColor, _FoamCol, foamAlpha); // lerp(深度渐变浅, 深度渐变深, 深度百分比)
                ...
                // 返回结果
                float3 finalRGB = dirLighting + envLighting + waterColor2 + emission; //  直接光照部分 + 环境光照部分 + 自发光部分
